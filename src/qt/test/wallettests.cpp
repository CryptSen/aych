...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<AychAmountField*>("payAmount")->setValue(amount);
    /* Aych: Disabled RBF UI
...
//
//     src/qt/test/test_aych-qt -platform xcb      # Linux
//     src/qt/test/test_aych-qt -platform windows  # Windows
//     src/qt/test/test_aych-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = AychUnits::formatWithUnit(unit, balance, false, AychUnits::separatorAlways);
    ...
    // Amount input
    AychAmountField* amountInput = receiveCoinsDialog.findChild<AychAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_aych-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
